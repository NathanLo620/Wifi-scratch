...
PEDCA_COUNTS    = list(range(0, 31, 5))                   # 0, 1, 2, ..., 30
DATA_RATE       = "1Mbps"
SIM_TIME        = 10.0
...
OUT_DIR = Path("/home/wmnlab/Desktop/ns-3.45/scratch/delay_pdf/VO+VI/fix_nsta30_VO+VI")
...
def vi_csv_name(n_pedca: int, data_rate: str, run_idx: int = None) -> str:
    tag = count_tag(n_pedca)
    if run_idx is not None:
        return f"{tag}_vi_delay_pdf_nSta{N_STA}_{data_rate}_run{run_idx}.csv"
    return f"{tag}_vi_delay_pdf_nSta{N_STA}_{data_rate}.csv"
...
def split_sta_csv_name(sta_type: str, ac: str, n_pedca: int,
                       data_rate: str, run_idx: int = None) -> str:
    """sta_type: 'pedca' or 'legacy'; ac: 'vo' or 'vi'."""
    tag = count_tag(n_pedca)
    base = f"{tag}_{sta_type}_sta_{ac}_delay_pdf_nSta{N_STA}_{data_rate}"
    if run_idx is not None:
        return f"{base}_run{run_idx}.csv"
    return f"{base}.csv"
...
def run_single_sim(n_pedca: int, data_rate: str,
    ...
    # Combined-STA per-AC CSVs (VO and VI)
    vo_csv_path = OUT_DIR / csv_name(n_pedca, data_rate, run_idx)
    vi_csv_path = OUT_DIR / vi_csv_name(n_pedca, data_rate, run_idx)
    rel_vo = str(vo_csv_path.relative_to(NS3_DIR))
    rel_vi = str(vi_csv_path.relative_to(NS3_DIR))

    # Per-STA-type (all-AC) CSVs
    pedca_csv_path = OUT_DIR / pedca_sta_csv_name(n_pedca, data_rate, run_idx)
    legacy_csv_path = OUT_DIR / legacy_sta_csv_name(n_pedca, data_rate, run_idx)
    rel_pedca = str(pedca_csv_path.relative_to(NS3_DIR))
    rel_legacy = str(legacy_csv_path.relative_to(NS3_DIR))

    # Split per-STA-type × per-AC CSVs
    split_paths = {
        ("pedca", "vo"): OUT_DIR / split_sta_csv_name("pedca", "vo", n_pedca, data_rate, run_idx),
        ("pedca", "vi"): OUT_DIR / split_sta_csv_name("pedca", "vi", n_pedca, data_rate, run_idx),
        ("legacy", "vo"): OUT_DIR / split_sta_csv_name("legacy", "vo", n_pedca, data_rate, run_idx),
        ("legacy", "vi"): OUT_DIR / split_sta_csv_name("legacy", "vi", n_pedca, data_rate, run_idx),
    }
    rel_split = {k: str(v.relative_to(NS3_DIR)) for k, v in split_paths.items()}
    ...
    args = (
        ...
        f"--voicePdfOutput={rel_vo} "
        f"--videoPdfOutput={rel_vi} "
        f"--pedcaStaDelayOutput={rel_pedca} "
        f"--legacyStaDelayOutput={rel_legacy} "
        f"--pedcaStaVoDelayOutput={rel_split[('pedca','vo')]} "
        f"--pedcaStaViDelayOutput={rel_split[('pedca','vi')]} "
        f"--legacyStaVoDelayOutput={rel_split[('legacy','vo')]} "
        f"--legacyStaViDelayOutput={rel_split[('legacy','vi')]} "
        f"--RngRun={run_idx + 1}"
    )
    ...
    try:
        ...
        if vo_csv_path.exists() and vo_csv_path.stat().st_size > 10:
            result_info["csv_path"] = vo_csv_path
            result_info["success"] = True
        if vi_csv_path.exists() and vi_csv_path.stat().st_size > 10:
            result_info["vi_csv_path"] = vi_csv_path
        ...
        for k, p in split_paths.items():
            if p.exists() and p.stat().st_size > 10:
                result_info["split_csv_paths"][k] = p
...
def parse_stats(stdout: str) -> dict:
    ...
    for line in lines:
        ...
        ext_map = {
            ...
            # Per-AC (VO/VI) global summary
            "VO_SUCC_COUNT": "vo_succ_count",
            "VO_FAIL_COUNT": "vo_fail_count",
            "VO_THROUGHPUT_MBPS": "vo_throughput_mbps",
            "VO_AVG_MAC_DELAY_US": "vo_avg_mac_delay_us",
            "VO_AVG_QUEUE_DELAY_US": "vo_avg_queue_delay_us",
            "VO_AVG_ACCESS_DELAY_US": "vo_avg_access_delay_us",
            "VO_MEDIAN_MAC_DELAY_US": "vo_median_mac_delay_us",
            "VO_P95_MAC_DELAY_US": "vo_p95_mac_delay_us",
            "VO_P99_MAC_DELAY_US": "vo_p99_mac_delay_us",
            "VI_SUCC_COUNT": "vi_succ_count",
            "VI_FAIL_COUNT": "vi_fail_count",
            "VI_THROUGHPUT_MBPS": "vi_throughput_mbps",
            "VI_AVG_MAC_DELAY_US": "vi_avg_mac_delay_us",
            "VI_AVG_QUEUE_DELAY_US": "vi_avg_queue_delay_us",
            "VI_AVG_ACCESS_DELAY_US": "vi_avg_access_delay_us",
            "VI_MEDIAN_MAC_DELAY_US": "vi_median_mac_delay_us",
            "VI_P95_MAC_DELAY_US": "vi_p95_mac_delay_us",
            "VI_P99_MAC_DELAY_US": "vi_p99_mac_delay_us",
            # Split STA-type × AC
            "PEDCA_STA_VO_COUNT": "pedca_sta_vo_count",
            "PEDCA_STA_VO_AVG_MAC_DELAY_US": "pedca_sta_vo_avg_mac_delay_us",
            "PEDCA_STA_VO_P95_MAC_DELAY_US": "pedca_sta_vo_p95_mac_delay_us",
            "PEDCA_STA_VI_COUNT": "pedca_sta_vi_count",
            "PEDCA_STA_VI_AVG_MAC_DELAY_US": "pedca_sta_vi_avg_mac_delay_us",
            "PEDCA_STA_VI_P95_MAC_DELAY_US": "pedca_sta_vi_p95_mac_delay_us",
            "LEGACY_STA_VO_COUNT": "legacy_sta_vo_count",
            "LEGACY_STA_VO_AVG_MAC_DELAY_US": "legacy_sta_vo_avg_mac_delay_us",
            "LEGACY_STA_VO_P95_MAC_DELAY_US": "legacy_sta_vo_p95_mac_delay_us",
            "LEGACY_STA_VI_COUNT": "legacy_sta_vi_count",
            "LEGACY_STA_VI_AVG_MAC_DELAY_US": "legacy_sta_vi_avg_mac_delay_us",
            "LEGACY_STA_VI_P95_MAC_DELAY_US": "legacy_sta_vi_p95_mac_delay_us",
        }
...
def average_stats(stats_list: list) -> dict:
    ...
    # Per-AC global + split STA-type × AC summary keys
    _extra_keys = [
        "vo_succ_count", "vo_fail_count", "vo_throughput_mbps",
        "vo_avg_mac_delay_us", "vo_avg_queue_delay_us", "vo_avg_access_delay_us",
        "vo_median_mac_delay_us", "vo_p95_mac_delay_us", "vo_p99_mac_delay_us",
        "vi_succ_count", "vi_fail_count", "vi_throughput_mbps",
        "vi_avg_mac_delay_us", "vi_avg_queue_delay_us", "vi_avg_access_delay_us",
        "vi_median_mac_delay_us", "vi_p95_mac_delay_us", "vi_p99_mac_delay_us",
        "pedca_sta_vo_count", "pedca_sta_vo_avg_mac_delay_us", "pedca_sta_vo_p95_mac_delay_us",
        "pedca_sta_vi_count", "pedca_sta_vi_avg_mac_delay_us", "pedca_sta_vi_p95_mac_delay_us",
        "legacy_sta_vo_count", "legacy_sta_vo_avg_mac_delay_us", "legacy_sta_vo_p95_mac_delay_us",
        "legacy_sta_vi_count", "legacy_sta_vi_avg_mac_delay_us", "legacy_sta_vi_p95_mac_delay_us",
    ]
    for k in _extra_keys:
        avg[k] = sum(s.get(k, 0) for s in stats_list) / n
...
def format_stats_text(avg: dict, n_runs: int) -> str:
    ...
    for ac in ("VO", "VI"):
        lc = ac.lower()
        if avg.get(f"{lc}_succ_count", 0) > 0:
            lines.append("")
            lines.append(f"--- {ac} Global Summary ---")
            lines.append(f"{ac}_SUCC_COUNT: {avg.get(f'{lc}_succ_count', 0):.1f}")
            lines.append(f"{ac}_FAIL_COUNT: {avg.get(f'{lc}_fail_count', 0):.1f}")
            lines.append(f"{ac}_THROUGHPUT_MBPS: {avg.get(f'{lc}_throughput_mbps', 0):.4f}")
            lines.append(f"{ac}_AVG_MAC_DELAY_US: {avg.get(f'{lc}_avg_mac_delay_us', 0):.3f}")
            lines.append(f"{ac}_AVG_QUEUE_DELAY_US: {avg.get(f'{lc}_avg_queue_delay_us', 0):.3f}")
            lines.append(f"{ac}_AVG_ACCESS_DELAY_US: {avg.get(f'{lc}_avg_access_delay_us', 0):.3f}")
            lines.append(f"{ac}_MEDIAN_MAC_DELAY_US: {avg.get(f'{lc}_median_mac_delay_us', 0):.3f}")
            lines.append(f"{ac}_P95_MAC_DELAY_US: {avg.get(f'{lc}_p95_mac_delay_us', 0):.3f}")
            lines.append(f"{ac}_P99_MAC_DELAY_US: {avg.get(f'{lc}_p99_mac_delay_us', 0):.3f}")

    # Split STA-type × AC comparison
    lines.append("")
    lines.append("--- Split STA-Type × AC Delay Comparison ---")
    for prefix_lc, label in [
        ("pedca_sta_vo", "P-EDCA STA × VO"),
        ("pedca_sta_vi", "P-EDCA STA × VI"),
        ("legacy_sta_vo", "Legacy STA × VO"),
        ("legacy_sta_vi", "Legacy STA × VI"),
    ]:
        cnt = avg.get(f"{prefix_lc}_count", 0)
        if cnt <= 0:
            continue
        lines.append(f"{label}:")
        lines.append(f"  COUNT: {cnt:.1f}")
        lines.append(f"  AVG_MAC_DELAY_US: {avg.get(f'{prefix_lc}_avg_mac_delay_us', 0):.3f}")
        lines.append(f"  P95_MAC_DELAY_US: {avg.get(f'{prefix_lc}_p95_mac_delay_us', 0):.3f}")
...
def aggregate_runs(run_results: list, n_pedca: int,
    ...
    vi_csv_paths = [r["vi_csv_path"] for r in run_results if r.get("vi_csv_path")]
    ...
    # Split STA-type × AC per-run paths
    split_collected = {("pedca", "vo"): [], ("pedca", "vi"): [],
                       ("legacy", "vo"): [], ("legacy", "vi"): []}
    for r in run_results:
        for k, p in r.get("split_csv_paths", {}).items():
            split_collected[k].append(p)

    avg_csv = OUT_DIR / csv_name(n_pedca, data_rate)
    avg_vi_csv = OUT_DIR / vi_csv_name(n_pedca, data_rate)
    avg_pedca_csv = OUT_DIR / pedca_sta_csv_name(n_pedca, data_rate)
    avg_legacy_csv = OUT_DIR / legacy_sta_csv_name(n_pedca, data_rate)
    avg_split_csv = {k: OUT_DIR / split_sta_csv_name(k[0], k[1], n_pedca, data_rate)
                     for k in split_collected}

    aggregate_jobs = [
        (csv_paths, avg_csv),
        (vi_csv_paths, avg_vi_csv),
        (pedca_csv_paths, avg_pedca_csv),
        (legacy_csv_paths, avg_legacy_csv),
    ]
    for k, paths in split_collected.items():
        aggregate_jobs.append((paths, avg_split_csv[k]))

    for paths, out in aggregate_jobs:
        if paths:
    ...
    return {
        ...
        "vi_csv_path": avg_vi_csv if vi_csv_paths else None,
        "pedca_csv_path": avg_pedca_csv if pedca_csv_paths else None,
        "legacy_csv_path": avg_legacy_csv if legacy_csv_paths else None,
        "split_csv_paths": {k: v for k, v in avg_split_csv.items()
                            if split_collected[k]},
        "stdout":    avg_stdout,
...
def _cdf_from_hist(mids, probs, zoom_xmax):
...
def plot_delay_pdf_overlay(results: dict, out_path: Path,
    ...
    Now plots BOTH AC_VO and AC_VI on the same axes (VO=solid, VI=dashed).
    """
    selected = [c for c in [0, 5, 10, 15, 20, 25, 30] if c in pedca_counts]
    if not selected:
    ...
    # Load both VO and VI histograms per nPedca
    loaded = {"vo": {}, "vi": {}}
    name_fn = {"vo": csv_name, "vi": vi_csv_name}
    all_series = []
    global_xmin = float("inf")

    for ac in ("vo", "vi"):
        for n_pedca in selected:
            cp = OUT_DIR / name_fn[ac](n_pedca, data_rate)
            if not cp.exists():
                continue
            try:
                mids, probs, xmin, xmax, bw = load_histogram(cp)
            except Exception:
                continue
            loaded[ac][n_pedca] = (mids, probs, bw)
            all_series.append((mids, probs))
            global_xmin = min(global_xmin, xmin)

    if not loaded["vo"] and not loaded["vi"]:
        plt.close(fig)
        return None
    ...
    ac_style = {"vo": ("solid", "VO"), "vi": ("dashed", "VI")}

    # PDF
    for ac in ("vo", "vi"):
        linestyle, ac_label = ac_style[ac]
        for n_pedca in selected:
            if n_pedca not in loaded[ac]:
                continue
            mids, probs, bw = loaded[ac][n_pedca]
            z_mids = [m for m in mids if m <= zoom_xmax]
            z_probs = [p for m, p in zip(mids, probs) if m <= zoom_xmax]
            if not z_mids:
                continue
            count_lbl = "Pure EDCA" if n_pedca == 0 else f"P-EDCA {n_pedca}/{N_STA}"
            label = f"{ac_label} | {count_lbl}"
            color = colors[n_pedca]
            ax_pdf.plot(z_mids, z_probs, linewidth=0.8, color=color,
    ...
    ax_pdf.set_title(
        f"VO + VI Delay PDF (zoomed to 95th pctl, VO=solid, VI=dashed)  -  nSta={N_STA}, {data_rate}{runs_label}",
        fontsize=12, fontweight="bold"
    )
    ax_pdf.grid(True, alpha=0.25, linestyle="--")
    ax_pdf.legend(loc="upper right", fontsize=7, ncol=2)
    ...
    for ac in ("vo", "vi"):
        ...
        for n_pedca in selected:
            if n_pedca not in loaded[ac]:
                continue
            mids, probs, bw = loaded[ac][n_pedca]
            cdf_mids, cdf_vals = _cdf_from_hist(mids, probs, zoom_xmax)
            if not cdf_mids:
                continue
            count_lbl = "Pure EDCA" if n_pedca == 0 else f"P-EDCA {n_pedca}/{N_STA}"
            label = f"{ac_label} | {count_lbl}"
            color = colors[n_pedca]
            ax_cdf.plot(cdf_mids, cdf_vals, linewidth=1.0, color=color,
                        linestyle=linestyle, label=label)
            _add_cdf_percentile_markers(ax_cdf, mids, probs, zoom_xmax,
                                        color, linestyle=":")
    ...
    ax_cdf.set_title(
        f"VO + VI Delay CDF (VO=solid, VI=dashed)  -  nSta={N_STA}, {data_rate}{runs_label}",
        fontsize=12, fontweight="bold"
    )
    ax_cdf.grid(True, alpha=0.25, linestyle="--")
    ax_cdf.legend(loc="lower right", fontsize=7, ncol=2)
...
def plot_sta_type_delay_overlay(out_path: Path, data_rate: str,
    ...
    across different nPedca values, with VO and VI plotted on the same axes
    (VO=solid, VI=dashed).
    """
    selected = [c for c in [0, 5, 10, 15, 20, 25, 30] if c in pedca_counts]
    if not selected:
    ...
    # Load split STA-type × AC histograms
    loaded = {"vo": {}, "vi": {}}
    all_series = []
    global_xmin = float("inf")
    for ac in ("vo", "vi"):
        for n_pedca in selected:
            cp = OUT_DIR / split_sta_csv_name(sta_type, ac, n_pedca, data_rate)
            if not cp.exists():
                continue
            try:
                mids, probs, xmin, xmax, bw = load_histogram(cp)
    ...
    x_95 = _compute_percentile_xlim(all_series, 0.95)
    zoom_xmax = x_95 * 1.10 if x_95 != float("inf") else 0.0
    if zoom_xmax <= 0:
        zoom_xmax = max(max(v[0]) for d in loaded.values() for v in d.values())
    runs_label = f", avg of {n_runs} runs" if n_runs > 1 else ""
    ...
    for ac in ("vo", "vi"):
        ...
        for n_pedca in selected:
            ...
            count_lbl = f"P-EDCA {n_pedca} / Legacy {n_legacy}"
            label = f"{ac_label} | {count_lbl}"
            color = colors[n_pedca]
            ax_pdf.plot(z_mids, z_probs, linewidth=0.8, color=color,
                        linestyle=linestyle, label=label)
    ...
    ax_pdf.set_ylabel("Probability", fontsize=11)
    ax_pdf.set_title(
        f"{type_label} VO+VI Delay PDF (VO=solid, VI=dashed)  -  nSta={N_STA}, {data_rate}{runs_label}",
        fontsize=12, fontweight="bold"
    )
    ax_pdf.grid(True, alpha=0.25, linestyle="--")
    ax_pdf.legend(loc="upper right", fontsize=7, ncol=2)
    ...
    ax_cdf.set_ylabel("Cumulative Probability", fontsize=11)
    ax_cdf.set_title(
        f"{type_label} VO+VI Delay CDF (VO=solid, VI=dashed)  -  nSta={N_STA}, {data_rate}{runs_label}",
        fontsize=12, fontweight="bold"
    )
    ax_cdf.grid(True, alpha=0.25, linestyle="--")
    ax_cdf.legend(loc="lower right", fontsize=7, ncol=2)
...
def plot_pedca_vs_legacy_cdf_per_count(out_path: Path, data_rate: str,
    ...
    overlaying 4 curves: P-EDCA-STA×VO, P-EDCA-STA×VI, Legacy-STA×VO,
    Legacy-STA×VI (plus a Pure-EDCA reference from n_pedca=0).
    """
    ...
    # Color/style per (sta_type, ac) — group by STA type via color, AC via linestyle.
    style_spec = [
        ("pedca", "vo", "#C44E52", "solid",  "P-EDCA STA × VO"),
        ("pedca", "vi", "#C44E52", "dashed", "P-EDCA STA × VI"),
        ("legacy", "vo", "#4C72B0", "solid", "Legacy STA × VO"),
        ("legacy", "vi", "#4C72B0", "dashed", "Legacy STA × VI"),
    ]
    ...
    with PdfPages(str(out_path)) as pdf:
        for n_pedca in mixed_counts:
            loaded = {}
            for sta_type, ac, color, linestyle, label in style_spec:
                cp = OUT_DIR / split_sta_csv_name(sta_type, ac, n_pedca, data_rate)
                if not cp.exists():
                    continue
                try:
                    ...
                    continue
                loaded[(sta_type, ac)] = (mids, probs, xmin, color, linestyle, label)

            # Pure EDCA reference: VO at n_pedca=0
            ref_vo = OUT_DIR / csv_name(0, data_rate)
            ref_vi = OUT_DIR / vi_csv_name(0, data_rate)
            for key, ref_csv, ref_ls, ref_lbl in [
                ("ref_vo", ref_vo, "solid",  "Pure EDCA × VO (n_pedca=0)"),
                ("ref_vi", ref_vi, "dashed", "Pure EDCA × VI (n_pedca=0)"),
            ]:
                if ref_csv.exists():
                    try:
                        m, p, x, _, _ = load_histogram(ref_csv)
                        loaded[key] = (m, p, x, "#555555", ref_ls, ref_lbl)
                    except Exception:
                        pass
            ...
            for key, val in loaded.items():
                mids, probs, _, color, linestyle, label = val
                cdf_mids, cdf_vals = _cdf_from_hist(mids, probs, zoom_xmax)
                if not cdf_mids:
                    continue
                ax.plot(cdf_mids, cdf_vals, linewidth=1.4,
                        color=color, label=label, linestyle=linestyle)
                _add_cdf_percentile_markers(ax, mids, probs, zoom_xmax,
                                            color, linestyle=":")
                plotted = True
            ...
            ax.set_title(
                f"P-EDCA vs Legacy × VO/VI Delay CDF  -  "
...
def plot_vo_vi_scalar_overlay(stats_path: Path, out_path: Path,
                               data_rate: str, pedca_counts: list,
                               n_runs: int = 1,
                               fig_width: float = 14.0,
                               fig_height: float = 12.0,
                               dpi: int = 200):
    """Three stacked subplots: (throughput, avg MAC delay, P95 MAC delay)
    each overlaying VO and VI vs nPedca."""
    metrics = [
        (("vo_throughput",        "vi_throughput_mbps"),    "Throughput (Mbps)", "Throughput"),
        (("vo_mac_delay",         "vi_avg_mac_delay_us"),   "Avg MAC Delay (us)", "Avg MAC Delay"),
        (("vo_p95_mac_delay_us",  "vi_p95_mac_delay_us"),   "P95 MAC Delay (us)", "P95 MAC Delay"),
    ]
    fig, axes = plt.subplots(len(metrics), 1, figsize=(fig_width, fig_height))
    if len(metrics) == 1:
        axes = [axes]
    runs_label = f" (avg of {n_runs} runs)" if n_runs > 1 else ""

    plotted_any = False
    for ax, ((vo_key, vi_key), ylabel, title) in zip(axes, metrics):
        vo = parse_stats_file_for_metric(stats_path, pedca_counts, vo_key)
        vi = parse_stats_file_for_metric(stats_path, pedca_counts, vi_key)
        if vo:
            x = sorted(vo.keys())
            y = [vo[k] for k in x]
            ax.plot(x, y, marker="o", linewidth=1.5, color="#C44E52", label="AC_VO")
            plotted_any = True
        if vi:
            x = sorted(vi.keys())
            y = [vi[k] for k in x]
            ax.plot(x, y, marker="s", linewidth=1.5, color="#4C72B0", label="AC_VI")
            plotted_any = True
        ax.set_xlabel("Number of P-EDCA STAs", fontsize=10)
        ax.set_ylabel(ylabel, fontsize=10)
        ax.set_title(f"{title}: VO vs VI  -  nSta={N_STA}, {data_rate}{runs_label}",
                     fontsize=11, fontweight="bold")
        ax.grid(True, alpha=0.3, linestyle="--")
        ax.legend(loc="best", fontsize=9)
        if vo or vi:
            all_x = sorted(set(list(vo.keys()) + list(vi.keys())))
            if all_x:
                ax.set_xticks(all_x)
...
def plot_split_sta_mac_delay(stats_path: Path, out_path: Path,
                              data_rate: str, pedca_counts: list,
                              n_runs: int = 1,
                              fig_width: float = 14.0,
                              fig_height: float = 8.0,
                              dpi: int = 200):
    """Overlay 4 curves vs nPedca: avg MAC delay for (P-EDCA × VO),
    (P-EDCA × VI), (Legacy × VO), (Legacy × VI)."""
    series = [
        ("pedca_sta_vo_avg_mac_delay_us", "P-EDCA STA × VO",  "#C44E52", "solid"),
        ("pedca_sta_vi_avg_mac_delay_us", "P-EDCA STA × VI",  "#C44E52", "dashed"),
        ("legacy_sta_vo_avg_mac_delay_us", "Legacy STA × VO", "#4C72B0", "solid"),
        ("legacy_sta_vi_avg_mac_delay_us", "Legacy STA × VI", "#4C72B0", "dashed"),
    ]
    fig, ax = plt.subplots(figsize=(fig_width, fig_height))
    runs_label = f" (avg of {n_runs} runs)" if n_runs > 1 else ""
    ...
    for key, label, color, linestyle in series:
        data = parse_stats_file_for_metric(stats_path, pedca_counts, key)
        if not data:
            continue
        # Filter zero-count rows (no samples)
        x = sorted([k for k, v in data.items() if v > 0])
        y = [data[k] for k in x]
        if not x:
            continue
    ...
    ax.set_xlabel("Number of P-EDCA STAs", fontsize=11)
    ax.set_ylabel("Avg MAC Delay (us)", fontsize=11)
    ax.set_title(
        f"P-EDCA vs Legacy x VO/VI Avg MAC Delay  -  nSta={N_STA}, {data_rate}{runs_label}",
        fontsize=12, fontweight="bold"
    )
    ax.grid(True, alpha=0.3, linestyle="--")
...
def main():
    ...
    individual_plots = [
        ...
        ("vo_mac_delay", "VO Avg MAC Delay (us)", "VO Avg MAC Delay vs P-EDCA STAs",
         f"vo_mac_delay_vs_pedca_count_{data_rate}.pdf"),
        # VI scalar metrics (parsed from EXTENDED_STATS block)
        ("vi_throughput_mbps", "VI Throughput (Mbps)", "VI Throughput vs P-EDCA STAs",
         f"vi_throughput_vs_pedca_count_{data_rate}.pdf"),
        ("vi_avg_mac_delay_us", "VI Avg MAC Delay (us)", "VI Avg MAC Delay vs P-EDCA STAs",
         f"vi_mac_delay_vs_pedca_count_{data_rate}.pdf"),
        ("vi_p95_mac_delay_us", "VI P95 MAC Delay (us)", "VI P95 MAC Delay vs P-EDCA STAs",
         f"vi_p95_mac_delay_vs_pedca_count_{data_rate}.pdf"),
    ]
    ...
    # 6b. P-EDCA vs Legacy × VO/VI CDF comparison, one page per n_pedca
    ...
    # 6c. VO vs VI scalar overlay (throughput / avg MAC delay / P95 MAC delay)
    vo_vi_overlay = OUT_DIR / f"vo_vi_scalar_overlay_{data_rate}.pdf"
    result = plot_vo_vi_scalar_overlay(
        stats_path, vo_vi_overlay, data_rate, pedca_counts, n_runs,
        args.fig_width, 12.0, args.dpi
    )
    if result:
        print(f"    ✔ {vo_vi_overlay.name}")
    else:
        print(f"    ⚠ No data for VO vs VI scalar overlay")

    # 6d. Split STA-type × AC avg MAC delay (4-curve scalar vs nPedca)
    split_scalar = OUT_DIR / f"split_sta_ac_mac_delay_{data_rate}.pdf"
    result = plot_split_sta_mac_delay(
        stats_path, split_scalar, data_rate, pedca_counts, n_runs,
        args.fig_width, args.fig_height, args.dpi
    )
    if result:
        print(f"    ✔ {split_scalar.name}")
    else:
        print(f"    ⚠ No data for split STA-type × AC MAC delay")
