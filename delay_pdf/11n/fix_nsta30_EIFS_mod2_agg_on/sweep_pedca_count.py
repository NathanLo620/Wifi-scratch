...
PEDCA_COUNTS    = list(range(0, 31))                   # 0, 1, 2, ..., 30
DATA_RATE       = "1Mbps"
SIM_TIME        = 10.0
...
OUT_DIR = Path("/home/wmnlab/Desktop/ns-3.45/scratch/delay_pdf/fix_nsta30_EIFS_mod2_agg_on")
...
def plot_delay_pdf_overlay(results: dict, out_path: Path,
    ...
    step = max(1, N_STA // 6); selected = [c for c in range(0, N_STA + 1, step) if c in pedca_counts]; selected = sorted(set(selected) | ({N_STA} & set(pedca_counts)))
    if not selected:
...
def plot_sta_type_delay_overlay(out_path: Path, data_rate: str,
    ...
    """
    step = max(1, N_STA // 6); selected = sorted(set([c for c in range(0, N_STA + 1, step) if c in pedca_counts]) | ({N_STA} & set(pedca_counts)))
    if not selected:
        selected = pedca_counts
