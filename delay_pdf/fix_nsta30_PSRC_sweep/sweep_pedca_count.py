...
PEDCA_COUNTS    = [0, 5, 10, 15, 20, 25, 30]           # P-EDCA STA counts to sweep
QSRC_VALUES     = [0, 1, 2, 3]                         # PSRC (PEDCA_CONSECUTIVE_ATTEMPT) sweep: 0..3 (QSRC fixed at 2)
DATA_RATE       = "1Mbps"
SIM_TIME        = 10.0
...
OUT_DIR = Path("/home/wmnlab/Desktop/ns-3.45/scratch/delay_pdf/fix_nsta30_PSRC_sweep")

# Header that holds the PSRC threshold (PEDCA_CONSECUTIVE_ATTEMPT).
...
# manually — all changes happen here. QSRC (PEDCA_RETRY_THRESHOLD) is left
# at whatever value the header currently holds (we fix QSRC=2 in the header).
...
# Module-level "current QSRC". Set by the main sweep loop and read by the
# filename helpers so that the existing call sites do not need a new arg.
CURRENT_QSRC: int = 0
...
def qsrc_tag(qsrc: int = None) -> str:
    # Despite the legacy function name, this tag now identifies the PSRC
    # (PEDCA_CONSECUTIVE_ATTEMPT) value being swept.
    q = CURRENT_QSRC if qsrc is None else qsrc
    return f"ps{q:02d}"

def qsrc_dir(qsrc: int = None) -> Path:
    """Per-QSRC subdirectory under OUT_DIR (created on demand)."""
...
def read_current_qsrc() -> int:
    """Read the current PEDCA_CONSECUTIVE_ATTEMPT (PSRC) value from the header file."""
...
def patch_qsrc(value: int):
    """Patch PEDCA_CONSECUTIVE_ATTEMPT (PSRC) in the header to `value`.
...
def plot_qsrc_overlay_for_npedca(out_path: Path, data_rate: str,
    ...
    # Legacy EDCA baseline: when n_pedca > 0, additionally load the n_pedca=0
    # case (no P-EDCA STAs active — PSRC value irrelevant) and overlay it as
    # a reference curve. Pulled from the first available PSRC subdir.
    baseline = None
    if sta_type in ("all", "legacy") and n_pedca > 0:
        for q in qsrc_values:
            base_csv = _series_csv_for(sta_type if sta_type == "all" else "all",
                                       q, 0, data_rate)
            if base_csv.exists():
                try:
                    b_mids, b_probs, b_xmin, _, _ = load_histogram(base_csv)
                    baseline = (b_mids, b_probs)
                    all_series.append((b_mids, b_probs))
                    global_xmin = min(global_xmin, b_xmin)
                    break
                except Exception:
                    pass
    ...
    if baseline is not None:
        b_mids, b_probs = baseline
        z_mids = [m for m in b_mids if m <= zoom_xmax]
        z_probs = [p for m, p in zip(b_mids, b_probs) if m <= zoom_xmax]
        if z_mids:
            ax_pdf.plot(z_mids, z_probs, linewidth=1.4, color="#222222",
                        linestyle="--", label="Legacy EDCA (baseline)")
    for q in qsrc_values:
        if q not in loaded:
            continue
    ...
    if baseline is not None:
        b_mids, b_probs = baseline
        full_total = sum(b_probs)
        if full_total > 0:
            cdf_mids, cdf_vals = [], []
            running = 0.0
            for m, p in zip(b_mids, b_probs):
                running += p
                if m <= zoom_xmax:
                    cdf_mids.append(m)
                    cdf_vals.append(running / full_total)
            if cdf_mids:
                ax_cdf.plot(cdf_mids, cdf_vals, linewidth=1.6, color="#222222",
                            linestyle="--", label="Legacy EDCA (baseline)")
                _add_cdf_percentile_markers(ax_cdf, b_mids, b_probs,
                                             zoom_xmax, "#222222",
                                             linestyle=":")
    for q in qsrc_values:
        if q not in loaded:
...
def run_one_qsrc(qsrc: int, data_rate: str, sim_time: float, bin_us: int,
                 pedca_counts: list, n_runs: int, workers: int, args):
    """Run the full P-EDCA-count sweep for a fixed QSRC value and
    generate the per-QSRC plots inside its subdirectory. Returns the
    stats file path so cross-QSRC code can re-use it later."""
    set_current_qsrc(qsrc)
    out_dir = qsrc_dir()

    print(f"\n┌──────────────────────────────────────────────────────────┐")
    print(f"│  PSRC = {qsrc:>2}  (PEDCA_CONSECUTIVE_ATTEMPT)                   │")
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                if done_count % max(1, total_sims // 40) == 0 or not r["success"]:
                    print(f"  [ps={qsrc} {done_count}/{total_sims}] {status} "
                          f"nPedca={n_pedca:>2} run={run_idx} {r['elapsed']:.1f}s")
            except Exception as e:
                print(f"  ✗ ps={qsrc} nPedca={n_pedca} run={run_idx} EXCEPTION: {e}")
    ...
    for n_pedca in sorted(raw_results.keys()):
        ...
        if run_list:
            ...
            print(f"  {status} ps={qsrc} nPedca={n_pedca:>2}  "
    ...
    # Per-QSRC plots into the per-QSRC subdir
...
def main():
    parser = argparse.ArgumentParser(
        description="P-EDCA Count × PSRC Sweep: fixed nSta=30, QSRC=2, vary P-EDCA count and PSRC"
    )
    parser.add_argument("--plot-only", action="store_true",
                        help="Skip simulations + builds, only re-plot existing data")
    parser.add_argument("--skip-rebuild", action="store_true",
                        help="Skip ns3 builds (assume binary already matches PSRC)")
    ...
    parser.add_argument("--counts", nargs="+", type=int, default=None,
                        help=f"P-EDCA STA counts to sweep (default: {PEDCA_COUNTS})")
    parser.add_argument("--psrc-values", "--qsrc-values", dest="qsrc_values",
                        nargs="+", type=int, default=None,
                        help=f"PSRC (PEDCA_CONSECUTIVE_ATTEMPT) values to sweep (default: {QSRC_VALUES})")
    ...
    print(f"\n╔══════════════════════════════════════════════════════════╗")
    print(f"║  P-EDCA × PSRC Sweep (Fixed nSta={N_STA}, QSRC=2)")
    print(f"║  P-EDCA counts = {pedca_counts}")
    print(f"║  PSRC values   = {qsrc_values}")
    ...
    # ── Outer PSRC loop with header patch + rebuild ──
    original_qsrc = None
    if not args.plot_only and not args.skip_rebuild:
        original_qsrc = read_current_qsrc()
        print(f"  Original PEDCA_CONSECUTIVE_ATTEMPT = {original_qsrc} "
              f"(will be restored at end)")

    try:
        if args.plot_only:
            for qsrc in qsrc_values:
                set_current_qsrc(qsrc)
                print(f"\n  [plot-only] PSRC={qsrc} subdir={qsrc_dir()}")
                for n_pedca in pedca_counts:
                    p = qsrc_dir() / csv_name(n_pedca, data_rate)
            ...
            for qsrc in qsrc_values:
                if not args.skip_rebuild:
                    patch_qsrc(qsrc)
                    rebuild_ns3()
                run_one_qsrc(qsrc, data_rate, sim_time, bin_us,
                             pedca_counts, n_runs, workers, args)
    finally:
        if original_qsrc is not None:
            try:
                patch_qsrc(original_qsrc)
                if not args.skip_rebuild:
                    print(f"\n  Restoring PEDCA_CONSECUTIVE_ATTEMPT = {original_qsrc}")
                    rebuild_ns3()
            except Exception as e:
                print(f"  ⚠ Failed to restore header: {e}")

    # ── Cross-PSRC overlay plots (the deliverable) ──
    print(f"\n{'─'*60}")
    print(f"  Generating cross-PSRC overlay plots (per nPedca)...")
    print(f"{'─'*60}")
    for n_pedca in pedca_counts:
        # combined VO (all STAs)
        out = OUT_DIR / f"vo_delay_pdf_cdf_psrc_overlay_p{n_pedca:02d}_{data_rate}.pdf"
        r = plot_qsrc_overlay_for_npedca(out, data_rate, n_pedca, qsrc_values,
                                          "all", n_runs,
                                          args.fig_width, 10.0, args.dpi)
        print(f"    {'✔' if r else '⚠'} {out.name}")

        # P-EDCA STA only (skip n_pedca=0)
        if n_pedca > 0:
            out = OUT_DIR / f"pedca_sta_delay_psrc_overlay_p{n_pedca:02d}_{data_rate}.pdf"
            r = plot_qsrc_overlay_for_npedca(out, data_rate, n_pedca, qsrc_values,
                                              "pedca", n_runs,
                                              args.fig_width, 10.0, args.dpi)
            print(f"    {'✔' if r else '⚠'} {out.name}")

        # Legacy STA only (skip n_pedca=N_STA)
        if n_pedca < N_STA:
            out = OUT_DIR / f"legacy_sta_delay_psrc_overlay_p{n_pedca:02d}_{data_rate}.pdf"
            r = plot_qsrc_overlay_for_npedca(out, data_rate, n_pedca, qsrc_values,
                                              "legacy", n_runs,
                                              args.fig_width, 10.0, args.dpi)
            print(f"    {'✔' if r else '⚠'} {out.name}")
    ...
    print(f"\n  Cross-PSRC overlay plots (top-level):")
    for f in sorted(OUT_DIR.glob(f"*psrc_overlay*.pdf")):
        print(f"    {f.name}  ({f.stat().st_size:,} bytes)")
    print(f"\n  Per-PSRC subdirectories:")
    for qsrc in qsrc_values:
        d = OUT_DIR / f"ps{qsrc:02d}"
        if d.exists():
            n_pdf = len(list(d.glob("*.pdf")))
            n_csv = len(list(d.glob(f"*_{data_rate}.csv")))
            print(f"    {d.name}/  ({n_pdf} pdfs, {n_csv} csvs)")
    print(f"{'═'*60}\n")
