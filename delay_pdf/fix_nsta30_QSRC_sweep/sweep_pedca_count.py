...
QSRC_VALUES     = list(range(0, 6))                    # QSRC threshold sweep: 0..5
DATA_RATE       = "1Mbps"
SIM_TIME        = 10.0
...
OUT_DIR = Path("/home/wmnlab/Desktop/ns-3.45/scratch/delay_pdf/fix_nsta30_QSRC_sweep")
...
def run_one_qsrc(qsrc: int, data_rate: str, sim_time: float, bin_us: int,
    ...
    print(f"\n┌──────────────────────────────────────────────────────────┐")
    print(f"│  QSRC = {qsrc:>2}  (PEDCA_RETRY_THRESHOLD)                       │")
...
def main():
    parser = argparse.ArgumentParser(
        description="P-EDCA Count × QSRC Sweep: fixed nSta=30, vary P-EDCA count and QSRC"
    )
    parser.add_argument("--plot-only", action="store_true",
                        help="Skip simulations + builds, only re-plot existing data")
    parser.add_argument("--skip-rebuild", action="store_true",
                        help="Skip ns3 builds (assume binary already matches QSRC)")
    ...
    print(f"\n╔══════════════════════════════════════════════════════════╗")
    print(f"║  P-EDCA × QSRC Sweep (Fixed nSta={N_STA})")
    ...
    # ── Outer QSRC loop with header patch + rebuild ──
    original_qsrc = None
    ...
    try:
        if args.plot_only:
            for qsrc in qsrc_values:
                set_current_qsrc(qsrc)
                print(f"\n  [plot-only] QSRC={qsrc} subdir={qsrc_dir()}")
                for n_pedca in pedca_counts:
        ...
        if original_qsrc is not None:
            try:
                ...
                if not args.skip_rebuild:
                    print(f"\n  Restoring PEDCA_RETRY_THRESHOLD = {original_qsrc}")
                    rebuild_ns3()
            except Exception as e:
                print(f"  ⚠ Failed to restore header: {e}")

    # ── Cross-QSRC overlay plots (the deliverable) ──
    print(f"\n{'─'*60}")
    print(f"  Generating cross-QSRC overlay plots (per nPedca)...")
    print(f"{'─'*60}")
    for n_pedca in pedca_counts:
        # combined VO (all STAs)
        out = OUT_DIR / f"vo_delay_pdf_cdf_qsrc_overlay_p{n_pedca:02d}_{data_rate}.pdf"
        ...
        if n_pedca > 0:
            out = OUT_DIR / f"pedca_sta_delay_qsrc_overlay_p{n_pedca:02d}_{data_rate}.pdf"
            r = plot_qsrc_overlay_for_npedca(out, data_rate, n_pedca, qsrc_values,
                                              "pedca", n_runs,
                                              args.fig_width, 10.0, args.dpi)
            print(f"    {'✔' if r else '⚠'} {out.name}")
        ...
        if n_pedca < N_STA:
            out = OUT_DIR / f"legacy_sta_delay_qsrc_overlay_p{n_pedca:02d}_{data_rate}.pdf"
            r = plot_qsrc_overlay_for_npedca(out, data_rate, n_pedca, qsrc_values,
                                              "legacy", n_runs,
                                              args.fig_width, 10.0, args.dpi)
            print(f"    {'✔' if r else '⚠'} {out.name}")
    ...
    print(f"\n  Cross-QSRC overlay plots (top-level):")
    for f in sorted(OUT_DIR.glob(f"*qsrc_overlay*.pdf")):
        print(f"    {f.name}  ({f.stat().st_size:,} bytes)")
    print(f"\n  Per-QSRC subdirectories:")
    for qsrc in qsrc_values:
        d = OUT_DIR / f"q{qsrc:02d}"
        if d.exists():
