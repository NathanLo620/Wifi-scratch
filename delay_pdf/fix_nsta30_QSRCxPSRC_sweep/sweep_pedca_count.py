...
PEDCA_COUNTS    = [5, 15, 30]                           # P-EDCA STA counts to run each QSRC×PSRC combo at
QSRC_VALUES     = list(range(0, 6))                    # QSRC threshold sweep: 0..5  (6 values)
PSRC_VALUES     = [1, 2, 3]                             # PSRC limit sweep: 1..3      (3 values)
BASELINE_NPEDCA = 0                                     # EDCA-only baseline = 0 P-EDCA STAs (QSRC/PSRC irrelevant)
DATA_RATE       = "1Mbps"
SIM_TIME        = 10.0
...
OUT_DIR = Path("/home/wmnlab/Desktop/ns-3.45/scratch/delay_pdf/fix_nsta30_QSRCxPSRC_sweep")
...
# Module-level "current QSRC / PSRC". Set by the main sweep loop and read by
# the filename helpers so that the existing call sites do not need a new arg.
# The combo tag is "q{QSRC}_s{PSRC}", which makes every existing name/dir
# helper (which routes through qsrc_tag/qsrc_dir) emit per-combo files in a
# per-combo subdirectory automatically.
CURRENT_QSRC: int = 0
CURRENT_PSRC: int = 1
...
def qsrc_tag(qsrc: int = None) -> str:
    """Combined QSRC×PSRC tag, e.g. 'q02_s01'. In baseline mode returns
    'edca_only'. The optional qsrc arg overrides only the QSRC component."""
    if BASELINE_MODE:
        return BASELINE_TAG
    q = CURRENT_QSRC if qsrc is None else qsrc
    return f"q{q:02d}_s{CURRENT_PSRC:02d}"

def combo_tag(qsrc: int, psrc: int) -> str:
    return f"q{qsrc:02d}_s{psrc:02d}"
...
def plot_combo_cdf(delay_type, type_label, csv_fn, n_pedca, data_rate,
                   qsrc, psrc, baseline_curve, n_runs,
                   fig_width=12.0, fig_height=6.0, dpi=200):
    """One CDF figure for a single (QSRC,PSRC) combo, fixed nPedca and one delay
    distribution. Overlays the EDCA-only baseline and marks the combo's median
    (P50) and P95. Appends a percentile row to PCTL_SUMMARY_ROWS."""
    ...
    ax.set_ylabel("Cumulative Probability", fontsize=11)
    runs_label = f", avg of {n_runs} runs" if n_runs > 1 else ""
    ax.set_title(f"{type_label} Delay CDF — QSRC={qsrc} × PSRC={psrc}\n"
    ...
    fig.tight_layout()
    out = qsrc_dir() / (f"{combo_tag(qsrc, psrc)}_{delay_type}_cdf_"
                        f"p{n_pedca:02d}_{data_rate}.pdf")
    fig.savefig(str(out), dpi=dpi)
    plt.close(fig)

    PCTL_SUMMARY_ROWS.append({
        "qsrc": qsrc, "psrc": psrc, "n_pedca": n_pedca, "delay_type": delay_type,
        "p50_us": pcts[0.5], "p95_us": pcts[0.95], "p99_us": pcts[0.99],
    })
    return pcts


def generate_combo_cdfs(qsrc, psrc, baseline_curve, data_rate,
                        pedca_counts, n_runs, args):
    """Per-combo deliverable: for each nPedca and each of the 3 delay
    distributions, emit a CDF figure with the EDCA-only baseline overlaid."""
    set_current_qsrc(qsrc)
    ...
    for n_pedca in pedca_counts:
        for delay_type, type_label, csv_fn in DELAY_TYPES:
            r = plot_combo_cdf(delay_type, type_label, csv_fn, n_pedca,
                               data_rate, qsrc, psrc, baseline_curve, n_runs,
                               args.fig_width, args.fig_height, args.dpi)
            if r:
                n += 1
    print(f"    ✔ {n} per-combo CDF figures for QSRC={qsrc} × PSRC={psrc}")
...
def write_pctl_summary(data_rate):
    ...
    with open(out, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["QSRC", "PSRC", "nPedca", "delay_type",
                    "P50_us", "P95_us", "P99_us"])
        for r in sorted(PCTL_SUMMARY_ROWS,
                        key=lambda r: (r["delay_type"], r["n_pedca"],
                                       r["qsrc"], r["psrc"])):
            w.writerow([r["qsrc"], r["psrc"], r["n_pedca"], r["delay_type"],
                        f"{r['p50_us']:.2f}", f"{r['p95_us']:.2f}",
                        f"{r['p99_us']:.2f}"])
    return out
...
def run_one_combo(qsrc: int, psrc: int, baseline_curve, data_rate: str,
                  sim_time: float, bin_us: int,
                  pedca_counts: list, n_runs: int, workers: int, args):
    """Run all nPedca points for a fixed (QSRC,PSRC) combo, write stats, and
    ...
    print(f"\n┌──────────────────────────────────────────────────────────┐")
    print(f"│  QSRC = {qsrc:>2} × PSRC = {psrc:>2}  (RETRY_THRESHOLD × CONSECUTIVE_ATTEMPT)")
    ...
    generate_combo_cdfs(qsrc, psrc, baseline_curve, data_rate,
                        pedca_counts, n_runs, args)
    return stats_path
...
def main():
    parser = argparse.ArgumentParser(
        description="P-EDCA QSRC×PSRC joint sweep: fixed nSta=30, "
    ...
    combos = [(q, s) for q in qsrc_values for s in psrc_values]

    print(f"\n╔══════════════════════════════════════════════════════════╗")
    print(f"║  P-EDCA QSRC×PSRC Joint Sweep (Fixed nSta={N_STA})")
    print(f"║  QSRC values   = {qsrc_values}")
    print(f"║  PSRC values   = {psrc_values}")
    print(f"║  → {len(combos)} combos, each run at nPedca = {pedca_counts}")
    ...
    # Preserve original header values; restore in finally.
    original_qsrc = None
    original_psrc = None
    if not args.plot_only and not args.skip_rebuild:
        original_qsrc = read_current_qsrc()
        original_psrc = read_current_psrc()
        print(f"  Original PEDCA_RETRY_THRESHOLD = {original_qsrc}, "
              f"PEDCA_CONSECUTIVE_ATTEMPT = {original_psrc} (restored at end)")
        ...
        if args.plot_only:
            ...
            for (qsrc, psrc) in combos:
                ...
                if stats_path.exists():
                    generate_per_qsrc_plots(stats_path, data_rate,
                                            pedca_counts, n_runs, args)
                generate_combo_cdfs(qsrc, psrc, base_curve, data_rate,
                                    pedca_counts, n_runs, args)
        else:
            # EDCA-only baseline: QSRC/PSRC irrelevant → build once, run once.
            ...
            for (qsrc, psrc) in combos:
                if not args.skip_rebuild:
                    ...
                    patch_psrc(psrc)
                    rebuild_ns3()
                run_one_combo(qsrc, psrc, base_curve, data_rate, sim_time,
                              bin_us, pedca_counts, n_runs, workers, args)
    finally:
        restored = []
        if original_qsrc is not None:
            try:
                patch_qsrc(original_qsrc); restored.append(f"QSRC={original_qsrc}")
            except Exception as e:
                print(f"  ⚠ Failed to restore QSRC: {e}")
        if original_psrc is not None:
            try:
                patch_psrc(original_psrc); restored.append(f"PSRC={original_psrc}")
            except Exception as e:
                print(f"  ⚠ Failed to restore PSRC: {e}")
        if restored and not args.skip_rebuild:
            print(f"\n  Restoring header ({', '.join(restored)}) and rebuilding...")
            try:
                rebuild_ns3()
            except Exception as e:
                print(f"  ⚠ Failed to rebuild after restore: {e}")
