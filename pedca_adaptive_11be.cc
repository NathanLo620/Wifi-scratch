/*
 * Adaptive P-EDCA: N STA Scenario  (802.11be / EHT)
 *
 * Same topology, traffic and statistics as pedca_verification_nsta_11be.cc, plus the
 * AP-side closed loop: stations piggyback buffer status and a low-latency indication on
 * their uplink frames, and every control period the AP re-chooses each station's
 * (CWds, QSRC threshold, PSRC limit) and advertises the table in the P-EDCA Parameter Set
 * element of its beacons.
 *
 * --adaptive=0 creates no controller and sets none of the feedback attributes, so the run
 * is identical to pedca_verification_nsta_11be with the same seed.
 *
 * Standard: WIFI_STANDARD_80211be, EhtMcs6 data rate, 5GHz 20MHz (ch36).
...
NS_LOG_COMPONENT_DEFINE("PedcaAdaptiveNSta");

// Controller trajectory, one row per control step, written to --trajOutput if given.
...
int main(int argc, char* argv[])
{
  ...
  uint32_t dsctsRepeat = 2;   // DS-CTS per Stage-1 attempt (1=single+AIFS, 2=dual+SIFS)
  ...
  double   busyHigh = 0.85;         // busy fraction that turns on the conservative gate
  double   collHigh = 0.90;         // DS-CTS collision rate that pushes CWds to its max
  double   collLow = 0.30;          // DS-CTS collision rate that drops CWds to zero
  uint32_t lliHigh = 5;             // per-STA LLI count marking delay pressure
  double   bsrHigh = 8.0;           // per-STA buffer status marking queue pressure
  uint32_t cwdsMax = 2;             // largest CWds the controller may choose
  uint32_t qsrcAggressive = 1;      // QSRC of the aggressive corner
  uint32_t psrcAggressive = 3;      // PSRC of the aggressive corner
  uint32_t qsrcConservative = 5;    // QSRC of the conservative corner
  uint32_t psrcConservative = 1;    // PSRC of the conservative corner
  bool     burstCollRate = true;    // decide on burst- rather than frame-level collisions
  double   ctrlStart = -1.0;        // when the controller starts; <0 means warmupTime
  std::string trajOutput = "";      // controller trajectory CSV (omit the flag to disable)
  ...
  cmd.AddValue("dsctsRepeat",
               "DS-CTS frames per Stage-1 attempt: 1=single DS-CTS then AIFS+backoff, "
               "2=dual DS-CTS a SIFS apart then SIFS+backoff (keeps backoff inside the 81us NAV)",
               dsctsRepeat);
  ...
  cmd.AddValue("ctrlStart",
               ...
               "Decoupled from warmupTime so the steady state after convergence can be measured "
               "on its own.",
               ctrlStart);
  ...
  cmd.AddValue("busyHigh","Busy fraction above which every STA is pushed to the conservative corner", busyHigh);
  cmd.AddValue("collHigh","DS-CTS collision rate above which CWds jumps to cwdsMax", collHigh);
  cmd.AddValue("collLow","DS-CTS collision rate below which CWds drops to 0", collLow);
  cmd.AddValue("lliHigh","Per-STA LLI count per period marking delay pressure", lliHigh);
  cmd.AddValue("bsrHigh","Per-STA buffer status (256B units) marking queue pressure", bsrHigh);
  cmd.AddValue("cwdsMax","Largest CWds the controller may choose", cwdsMax);
  cmd.AddValue("qsrcAggressive","QSRC threshold of the aggressive corner", qsrcAggressive);
  cmd.AddValue("psrcAggressive","PSRC limit of the aggressive corner", psrcAggressive);
  cmd.AddValue("qsrcConservative","QSRC threshold of the conservative corner", qsrcConservative);
  cmd.AddValue("psrcConservative","PSRC limit of the conservative corner", psrcConservative);
  cmd.AddValue("burstCollRate",
               "Measure DS-CTS collisions per Stage-1 burst (1) rather than per frame (0). With "
               "dual DS-CTS a burst that lost only its first frame worked as designed.",
               burstCollRate);
  ...
  // ── Adaptive P-EDCA feedback path. Everything here is inside the adaptive branch so that
  //    --adaptive=0 leaves the simulation bit-identical to pedca_verification_nsta_11be.
  if (adaptive)
  {
      // Stations only report their queue size when asked to.
      Config::SetDefault("ns3::QosFrameExchangeManager::SetQueueSize", BooleanValue(true));
      // The 20 ms default expires a buffer status report long before a 100 ms control step
      // reads it, so every station would look idle.
  ...
  // AP Setup. Note PedcaControl, not PedcaSupported: the latter would run the AP's own VO
  // management frames through the P-EDCA sender and poison the EDCA parameters it advertises.
  WifiMacHelper mac;
  ...
  // ── AP-side controller. Both arms seed the stations with the same theta above, so the
  //    adaptive arm starts where the fixed arm stays.
  ...
  if (adaptive)
  {
      ...
      if (!trajOutput.empty())
      {
          ...
          {
              std::cout << "[WARN] Failed to open trajectory CSV: " << trajOutput << "\n";
          }
      }
...
// ── P-EDCA Detailed Trace ──
  ...
  uint32_t totalFailDidNotInitiateTxop = 0;
  ...
  for (uint32_t i = 0; i < nSta; ++i)
  {
      ...
      uint32_t failNoTxop = qosFem->GetPedcaFailDidNotInitiateTxop();
      ...
      totalFailDidNotInitiateTxop += failNoTxop;
  ...
  std::cout << "P-EDCA Fail Did Not Initiate TXOP: "
            << totalFailDidNotInitiateTxop << "\n";
  ...
  // ── [Stat 2b] DS-CTS as transmitted vs as heard by the AP ──
  //    With --dsctsRepeat=2 each Stage-1 attempt puts two frames on air, so framesTx should be
  //    twice the attempt count and the AP's frame count should match it. An AP frame count of
  //    zero means the receive-side counter is never reached.
  {
    uint32_t totalDsCtsFramesTx = 0;
    for (uint32_t i = 0; i < nPedcaSta; ++i)
    {
        Ptr<WifiNetDevice> wDev = DynamicCast<WifiNetDevice>(staDevices.Get(i));
        if (!wDev) continue;
        auto qFem = DynamicCast<QosFrameExchangeManager>(wDev->GetMac()->GetFrameExchangeManager(0));
        if (!qFem) continue;
        totalDsCtsFramesTx += qFem->GetDsCtsFramesTx();
    }

    Ptr<WifiNetDevice> apDev = DynamicCast<WifiNetDevice>(apDevices.Get(0));
    auto apFem = DynamicCast<QosFrameExchangeManager>(apDev->GetMac()->GetFrameExchangeManager(0));

    std::cout << "\n--- [Stat 2b] DS-CTS Sent vs Heard by the AP (dsctsRepeat=" << dsctsRepeat << ") ---\n";
    std::cout << "  STA side: bursts(attempts)=" << totalDsCtsSent
              << "  frames=" << totalDsCtsFramesTx << "\n";
    if (apFem)
    {
        std::cout << "  AP side:  bursts=" << apFem->GetDsCtsBurstRxCount()
                  << "  frames=" << apFem->GetDsCtsRxCount()
                  << "  lliRx=" << apFem->GetLliRxCount() << "\n";
    }
    if (pedcaController)
    {
  ...
  std::cout << "\n--- [Stat 5] P-EDCA Kickback-to-EDCA Probability ---\n";
  {
    ...
    if (totalDsCtsSent > 0) {
      ...
      std::cout << "    Did Not Initiate TXOP: " << totalFailDidNotInitiateTxop << " ("
                << ((double)totalFailDidNotInitiateTxop / totalDsCtsSent * 100.0)
                << " %)\n";
    }
  }
