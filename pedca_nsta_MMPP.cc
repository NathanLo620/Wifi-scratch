 ...
 * - Traffic: 2-state MMPP per STA (mean load = --dataRate), VO only
...
// ──────────────────────────────────────────────────────────────────────
//  MMPP (Markov-Modulated Poisson Process) UDP traffic source
//  ---------------------------------------------------------------------
//  A 2-state continuous-time Markov chain modulates the arrival rate of an
//  otherwise-Poisson source ("switched Poisson process"). The chain dwells
//  in each state for an exponentially distributed time (mean m_dwell) and,
//  while in a state, packets arrive as a Poisson process at that state's
//  rate. Because the Poisson process is memoryless, the time to the next
//  arrival is re-drawn from the new state's rate at every state transition,
//  which is exact for an MMPP.
//
//  The two rates are chosen symmetric about the requested mean rate
//  (lambda_low = 0.5*mean, lambda_high = 1.5*mean) with symmetric dwell
//  times, so both states are equally likely and the *time-average* arrival
//  rate equals the CBR/Poisson mean rate — keeping the average offered load
//  identical across traffic models. Packet layout (SeqTsHeader + filler) and
//  IP ToS handling match UdpClient.
// ──────────────────────────────────────────────────────────────────────
class MmppUdpClient : public Application
{
public:
  static TypeId GetTypeId();
  MmppUdpClient();
  ~MmppUdpClient() override = default;

  // meanRate: time-average packets per second of the MMPP.
...
private:
  ...
  double                         m_meanInterval[2] {0.0, 0.0}; // mean inter-arrival per state (s)
  double                         m_dwell {0.1};                // mean state sojourn time (s)
  uint8_t                        m_state {0};

  EventId                        m_sendEvent;
  EventId                        m_switchEvent;
  Ptr<ExponentialRandomVariable> m_unitExp;   // unit-mean exponential, scaled per state
  Ptr<ExponentialRandomVariable> m_dwellExp;  // state sojourn time
};

TypeId
MmppUdpClient::GetTypeId()
{
  static TypeId tid = TypeId("MmppUdpClient")
                          .SetParent<Application>()
                          .SetGroupName("Applications")
                          .AddConstructor<MmppUdpClient>();
  return tid;
}

MmppUdpClient::MmppUdpClient()
{
  m_unitExp  = CreateObject<ExponentialRandomVariable>();
  m_unitExp->SetAttribute("Mean", DoubleValue(1.0));
  m_dwellExp = CreateObject<ExponentialRandomVariable>();
}

void
MmppUdpClient::Setup(Address peer, uint32_t pktSize, double meanRate,
                     uint8_t tos, uint32_t maxPackets)
{
  m_peer = peer;
  m_pktSize = pktSize;
  ...
  // Symmetric two-rate MMPP: time-average rate = meanRate.
  double lambdaLow  = 0.5 * meanRate;
  double lambdaHigh = 1.5 * meanRate;
  m_meanInterval[0] = 1.0 / lambdaLow;
  m_meanInterval[1] = 1.0 / lambdaHigh;
  m_dwellExp->SetAttribute("Mean", DoubleValue(m_dwell));
}

void
MmppUdpClient::ScheduleSend()
{
  double interval = m_unitExp->GetValue() * m_meanInterval[m_state];
  m_sendEvent = Simulator::Schedule(Seconds(interval), &MmppUdpClient::Send, this);
}

void
MmppUdpClient::StartApplication()
{
  if (!m_socket)
  {
  ...
  m_state = 0;
  ScheduleSend();
  m_switchEvent = Simulator::Schedule(Seconds(m_dwellExp->GetValue()),
                                      &MmppUdpClient::SwitchState, this);
}

void
MmppUdpClient::SwitchState()
{
  m_state = (m_state == 0) ? 1 : 0;
  // Memoryless: re-draw the next arrival from the new state's rate.
...
void
MmppUdpClient::StopApplication()
{
  if (m_sendEvent.IsPending())   { Simulator::Cancel(m_sendEvent); }
  if (m_switchEvent.IsPending()) { Simulator::Cancel(m_switchEvent); }
...
int main(int argc, char* argv[])
{
  ...
  // Traffic model: 2-state MMPP (Markov-Modulated Poisson Process). The mean
  // packet rate matches the CBR rate derived from --dataRate (packetsPerSecond
  // below), so the average offered load is unchanged; arrivals are Poisson at
  // a rate that is modulated between a low and a high value by a 2-state
  // Markov chain.
  ...
  for (uint32_t i = 0; i < nSta; ++i)
  {
      Ptr<MmppUdpClient> client = CreateObject<MmppUdpClient>();
