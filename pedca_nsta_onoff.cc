 ...
 * - Traffic: bursty ON/OFF source per STA (mean load = --dataRate), VO only
...
int main(int argc, char* argv[])
{
  ...
  // Traffic model: bursty ON/OFF source. ON and OFF durations are
  // exponentially distributed with equal means (50% duty cycle); during the
  // ON state the source transmits at 2x the --dataRate so the *average*
  // offered load equals the original CBR rate. Only the traffic model differs
  // from the baseline simulation; packet size / ToS / timing are unchanged.
  DataRate rate(dataRate);
  uint64_t onRateBps = 2 * rate.GetBitRate();   // 50% duty cycle -> mean = dataRate
  const double meanOnOffSec = 0.1;              // mean ON / OFF burst length (s)

  Ptr<UniformRandomVariable> startRv = CreateObject<UniformRandomVariable>();

  std::ostringstream onTimeSs, offTimeSs;
  onTimeSs  << "ns3::ExponentialRandomVariable[Mean=" << meanOnOffSec << "]";
  offTimeSs << "ns3::ExponentialRandomVariable[Mean=" << meanOnOffSec << "]";

  for (uint32_t i = 0; i < nSta; ++i)
  {
      OnOffHelper onoff("ns3::UdpSocketFactory",
                        InetSocketAddress(apIf.GetAddress(0), basePort + voAc));
      onoff.SetAttribute("OnTime",     StringValue(onTimeSs.str()));
      onoff.SetAttribute("OffTime",    StringValue(offTimeSs.str()));
      onoff.SetAttribute("DataRate",   DataRateValue(DataRate(onRateBps)));
      onoff.SetAttribute("PacketSize", UintegerValue(payloadSize));
      onoff.SetAttribute("Tos",        UintegerValue(voTos));
