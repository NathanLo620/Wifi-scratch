...
TosForAc(uint8_t ac)
{
  switch (ac)
  {
  case kAcBk: return 0x20; // BK
  case kAcVi: return 0xa0; // VI
  case kAcVo: return 0xc0; // VO
  case kAcBe:
...
void
ApplyPedcaProfile(const NodeContainer &nodes)
{
  const std::array<std::string, 4> txopNames = {
      "BE_Txop",
      "BK_Txop",
      "VI_Txop",
      "VO_Txop",
  ...
  const std::array<EdcaProfile, 4> profiles = {
      EdcaProfile{31, 1023, 7}, // BE
      EdcaProfile{63, 1023, 7}, // BK
      EdcaProfile{7, 15, 2},    // VI
      EdcaProfile{3, 7, 1},     // VO
  };
  ...
  for (uint32_t i = 0; i < nodes.GetN(); ++i)
  {
    ...
    for (size_t ac = 0; ac < txopNames.size(); ++ac)
    {
      PointerValue ptr;
      bool found = wifiMac->GetAttributeFailSafe(txopNames[ac], ptr);
      if (!found)
      {
        continue;
      }
      ...
      Ptr<QosTxop> qosTxop = DynamicCast<QosTxop>(txopObj);
      if (qosTxop)
      {
        qosTxop->SetMinCw(profiles[ac].minCw);
        qosTxop->SetMaxCw(profiles[ac].maxCw);
        qosTxop->SetAifsn(profiles[ac].aifsn);
      }
      else
      {
        ...
        if (txop)
        {
          txop->SetMinCw(profiles[ac].minCw);
          txop->SetMaxCw(profiles[ac].maxCw);
        }
      }
    }
  }
...
main(int argc, char *argv[])
{
  ...
  ApplyPedcaProfile(sta);
  ApplyPedcaProfile(ap);
