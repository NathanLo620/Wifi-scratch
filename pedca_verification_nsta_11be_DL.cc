/*
 * P-EDCA Verification: N STA Scenario — DOWNLINK  (802.11be / EHT)
 *
 * Downlink counterpart of pedca_verification_nsta_11be.cc.
 * The ONLY functional change vs. the uplink file is the traffic direction:
 *   - Uplink   : each STA is a UDP client -> AP is the UDP server  (STAs transmit)
 *   - Downlink : AP is the UDP client     -> each STA is a UDP server (AP transmits)
 *
 * Because P-EDCA is a *transmitter-side* mechanism (it is triggered on the FEM of
 * whoever originates the AC_VO frames), in downlink the AP is the sole transmitter,
 * so P-EDCA is enabled on the AP's FrameExchangeManager. The per-STA P-EDCA/legacy
 * split (pedcaRatio -> first nPedcaSta STAs are "P-EDCA STAs") is preserved exactly
 * as in the reference file so the two STA kinds still exist; the P-EDCA mechanism
 * counters are read from the AP (the downlink transmitter).
 *
 * Standard: WIFI_STANDARD_80211be, EhtMcs5 data rate, 5GHz 20MHz (ch36), GI 1600 ns.
 * No MLO (single 5GHz link).
 *
 * Use Case:
 * - Scalability test: Simulate N STAs
 * - Compare Delay, Throughput, Retransmission, Packet Loss across Access Categories
 * - Traffic: 0.5 Mbps per STA, VO only (downlink AP -> STA)
 * - STA1..N: Placed at fixed distance (5m) from AP
 * - First nPedcaSta STAs are "P-EDCA STAs" (PedcaSupported=true)
 *
...
NS_LOG_COMPONENT_DEFINE("PedcaVerificationNStaDL");
...
int main(int argc, char* argv[])
{
  ...
  std::string voicePdfOutput = "scratch/delay_pdf/pedca_vo_delay_pdf_DL.csv";
  ...
  std::string clogFile = "scratch/pedca_stage2_stats_DL.log";
  ...
  // Number of P-EDCA STAs (destination STAs served by the P-EDCA path in downlink).
  uint32_t nPedcaSta = static_cast<uint32_t>(nSta * pedcaRatio);
  // In downlink the AP is the sole transmitter, so P-EDCA is enabled on the AP
  // whenever there is at least one P-EDCA STA to serve.
  bool apPedcaEnabled = (nPedcaSta > 0);
  ...
  if (verbose) {
    LogComponentEnable("PedcaVerificationNStaDL", LOG_LEVEL_INFO);
  }
  ...
  // AP Setup — the AP is the downlink transmitter, so P-EDCA is enabled here.
  WifiMacHelper mac;
  mac.SetType("ns3::ApWifiMac",
              "Ssid", SsidValue(ssid),
              "QosSupported", BooleanValue(true),
              "PedcaSupported", BooleanValue(apPedcaEnabled));
  NetDeviceContainer apDevices = wifi.Install(phy, mac, wifiApNode);

  // STA Setup - P-EDCA enabled for first nPedcaSta STAs (preserves the two STA kinds).
  ...
  // Apply CWds / QSRC-threshold / PSRC-limit to the AP's FEM (the downlink
  // transmitter that actually runs P-EDCA). All three are runtime parameters.
  if (apPedcaEnabled)
  {
      auto apW = DynamicCast<WifiNetDevice>(apDevices.Get(0));
      auto apFem = DynamicCast<QosFrameExchangeManager>(apW->GetMac()->GetFrameExchangeManager(0));
      if (apFem) {
          apFem->SetCwds(cwds);
          apFem->SetQsrc(static_cast<uint16_t>(qsrc));
          apFem->SetPsrc(static_cast<uint8_t>(psrc));
      }
  }

  // Also apply the same P-EDCA parameters to every P-EDCA STA's FEM, mirroring the
  // reference file. (STAs are receivers in downlink, so this is inert for DL data,
  // but it keeps the STA configuration identical to the uplink verification.)
  ...
  // Traffic: DOWNLINK — UDP Server on each STA (receiver), UDP Client on AP (VO only)
  uint16_t basePort = 5000;
  constexpr uint8_t voAc = 3;
  constexpr uint8_t voTos = 0xC0;

  // One UDP server per STA: each STA receives its own downlink VO flow.
  ApplicationContainer serverApps;
  for (uint32_t i = 0; i < nSta; ++i)
  {
      UdpServerHelper server(basePort + voAc);
      ApplicationContainer app = server.Install(wifiStaNodes.Get(i));
      app.Start(Seconds(0.5));
      app.Stop(Seconds(simTime));
      serverApps.Add(app);
  }

  // Clients on AP: the AP sends VO traffic to each STA (downlink).
  ...
  for (uint32_t i = 0; i < nSta; ++i)
  {
      UdpClientHelper client(staIf.GetAddress(i), basePort + voAc);
  ...
  {
    std::clog << "============ P-EDCA Stage 2 Diagnostic Log (DOWNLINK) ============\n";
    std::clog << "nSta=" << nSta << "  P-EDCA STAs=" << nPedcaSta
              << "  AP P-EDCA=" << (apPedcaEnabled ? "ON" : "OFF")
...
// ── P-EDCA Detailed Trace (read from the AP: the downlink transmitter) ──
  ...
  std::cout << "\n--- AP (Downlink Transmitter) P-EDCA Detail ---\n";
  {
      Ptr<WifiNetDevice> apW = DynamicCast<WifiNetDevice>(apDevices.Get(0));
      auto qosFem = apW ? DynamicCast<QosFrameExchangeManager>(apW->GetMac()->GetFrameExchangeManager(0))
                        : nullptr;
      if (qosFem) {
          totalDsCtsSent         = qosFem->GetDsCtsCount();
          totalStage2Entry       = qosFem->GetStage2EntryCount();
          totalStage2TxStart     = qosFem->GetStage2TxStartCount();
          totalPedcaSuccess      = qosFem->GetPedcaSuccessCount();
          totalEdcaVoSuccess     = qosFem->GetEdcaVoSuccessCount();
          totalFailRtsCtsTimeout = qosFem->GetPedcaFailRtsCtsTimeout();
          totalFailRtsCollision  = qosFem->GetPedcaFailRtsCollision();
          totalFailTimingExpired = qosFem->GetPedcaFailTimingExpired();
          totalFailDeferral      = qosFem->GetPedcaFailDeferral();

          std::cout << "  AP: DS-CTS=" << totalDsCtsSent
                    << " S2Entry=" << totalStage2Entry << " S2Tx=" << totalStage2TxStart
                    << " PedcaOK=" << totalPedcaSuccess << " EdcaOK=" << totalEdcaVoSuccess
                    << " FailCTS=" << totalFailRtsCtsTimeout << " FailColl=" << totalFailRtsCollision
                    << " FailExp=" << totalFailTimingExpired << " Defer=" << totalFailDeferral << "\n";
      }
  }

  uint32_t totalVoTxAtTx = totalPedcaSuccess + totalEdcaVoSuccess;
  double avgPedcaTxRatio = (totalVoTxAtTx > 0) ? ((double)totalPedcaSuccess / totalVoTxAtTx) : 0.0;
  double avgPedcaSuccessRate = (totalDsCtsSent > 0) ? ((double)totalPedcaSuccess / totalDsCtsSent) : 0.0;

  std::cout << "\n=== General Statistics ===\n";
  std::cout << "P-EDCA Ratio: " << pedcaRatio << "\n";
  std::cout << "P-EDCA STAs: " << nPedcaSta << "/" << nSta << " (AP P-EDCA=" << (apPedcaEnabled ? "ON" : "OFF") << ")\n";
  std::cout << "P-EDCA Share (P-EDCA Tx/Total VO Tx at AP): " << (avgPedcaTxRatio * 100.0) << " %\n";
  std::cout << "P-EDCA Attempt Success Rate: " << (avgPedcaSuccessRate * 100.0) << " %\n";
  ...
  // ── Build node-ID sets for the downlink transmitter ──
  //    In downlink the AP is the sole transmitter, so WifiTxStatsHelper attributes
  //    every data record to the AP node. We classify the AP's downlink traffic as
  //    "P-EDCA" when the AP runs P-EDCA, otherwise "Legacy". (Per-destination-STA
  //    MAC-delay split is not available: the helper records only the sending node.)
  std::set<uint32_t> pedcaNodeIds, legacyNodeIds;
  uint32_t apNodeId = wifiApNode.Get(0)->GetId();
  if (apPedcaEnabled)
      pedcaNodeIds.insert(apNodeId);
  else
      legacyNodeIds.insert(apNodeId);
  ...
  // [Stat 1] Partition failures by transmitter type
  ...
  // [Stat 1] Delay partitioned by transmitter type (P-EDCA vs Legacy)
  ...
  // [Stat 4 extended] P-EDCA one-shot success (zero retransmissions at MAC level)
  ...
  // =====================================================================
  // ══════ Extended P-EDCA vs Legacy Statistics (Stats 1-7) ══════
  //   In downlink the AP is the transmitter; the "P-EDCA" vs "Legacy"
  //   partition reflects whether the AP transmitter runs P-EDCA.
  // =====================================================================
  std::cout << "\n=== Extended P-EDCA vs Legacy Statistics (Downlink) ===\n";

  // ── [Stat 1] Delay partitioned by transmitter type ──
  std::cout << "\n--- [Stat 1] Delay by Transmitter Type ---\n";
  {
    // P-EDCA (AP downlink transmitter running P-EDCA)
    ...
    std::cout << "P-EDCA Downlink (AP, serving " << nPedcaSta << " P-EDCA STAs):\n";
    ...
    // Legacy (AP downlink transmitter running plain EDCA)
    ...
    std::cout << "Legacy EDCA Downlink (AP, serving " << nLegacy << " Legacy STAs):\n";
  ...
  // ── [Stat 2] P-EDCA attempt count (DS-CTS sent) at the AP transmitter ──
  std::cout << "\n--- [Stat 2] P-EDCA Attempt Count (DS-CTS Sent) ---\n";
  std::cout << "  AP: " << totalDsCtsSent << " P-EDCA attempts (downlink transmitter)\n";
  ...
  //    = Packets with 0 retransmissions / total successes
  std::cout << "\n--- [Stat 4] EDCA One-Shot Success Ratio (0 retransmissions) ---\n";
  {
    ...
    std::cout << "  Legacy DL: " << legacyStaZeroRetx << " / " << legacyStaSuccCount
              << " = " << legacyOneShot << " %\n";
    std::cout << "  P-EDCA DL: " << pedcaStaZeroRetx << " / " << pedcaStaSuccCount
  ...
  // ── P-EDCA / Legacy Delay Histogram CSV ──
  ...
  writeDelayHistCsv(pedcaStaMacDelayVec, pedcaStaDelayOutput, "P-EDCA DL Delay PDF");
  writeDelayHistCsv(legacyStaMacDelayVec, legacyStaDelayOutput, "Legacy DL Delay PDF");
